...
pub trait ThermogramNdarrayExt: ThermogramTrait {
    /// Thermal data in celsius, as a height × width array.
    fn thermal_ndarray(&self) -> Array2<f32> {
        let thermal = self.thermal().as_ref();
        let pixels: Vec<f32> = thermal.pixels().collect();
    ...
    fn render_ndarray(&self, min_temp: f32, max_temp: f32, palette: &[[f32; 3]]) -> Array3<u8> {
